...
Phase 0 storage policy is a conservative row-level contract, not a full industrial throughput proof: inbound placement spreads work across FIFO rows while preserving contiguous fill inside each row, inbound shuttles back out toward the infeed side after dropoff, outbound drains without hidden compaction, stored pallets do not block shuttle pass-through under the load, and all reservation capacities remain fixed at `1`. Lift/lower timing defaults to `0.05s` so Phase 0 pressure runs focus on shuttle traffic and storage behavior unless lift timing is deliberately raised from the dashboard.
