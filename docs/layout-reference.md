...
- FIFO is primarily a storage/retrieval policy: the shuttle can travel both directions inside a row, inbound places from the right-side infeed direction into the deepest reachable empty cell, and outbound picks from the left-side outfeed direction. Phase 0 constrains the right-side row spine to inbound feed movement and serializes outbound left-FIFO work until calibrated passing/escape geometry is modeled.
