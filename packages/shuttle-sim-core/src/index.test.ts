...
describe('shuttle phase 0 SimCore', () => {
  ...
  it('primes the top-lift inbound source buffers and keeps SKU columns clear of idle parking', () => {
    ...
    expect(inboundTasks.map((task) => task.liftPortNodeId)).toEqual([
      'lift-01-inbound',
      'lift-02-inbound',
      'lift-01-inbound',
      'lift-02-inbound',
  ...
  it('routes inbound pickup to the nearest yellow-grid lift dock without entering the lift workcell', () => {
    const scenario = createInboundMvpBaselineScenario();
    const sim = new ShuttleSimCore(scenario);
    sim.start();
    sim.advanceByInPlace(3);
    const state = sim.getState();
    ...
    expect(task?.pickupNodeId).toBe('module-01-spine-top-a');
    expect(task?.liftPortNodeId).toBe('lift-01-inbound');
    expect(vehicle?.routeNodeIds.at(-1)).toBe('module-01-spine-top-a');
    const route = vehicle?.routeNodeIds ?? [];
    expect(route).toContain('module-01-spine-top-a');
  ...
  it('allows same-direction top-lift spine following when the leading shuttle has enough headway', () => {
    ...
    const state = sim.getState();
    const bottomLaneFollower = state.vehicles.find((vehicle) =>
      vehicle.currentEdgeId !== null &&
      /^column-bottom-[ab]-c\d+$/.test(vehicle.currentNodeId) &&
      /^column-bottom-[ab]-c\d+$/.test(vehicle.targetNodeId ?? '') &&
      vehicle.routeNodeIds.every((nodeId) => !/^lift-\d{2}-outbound-queue-/.test(nodeId))
    );
    expect(bottomLaneFollower).toBeDefined();
    expect(bottomLaneFollower?.waitReason).toBeNull();
    expect(bottomLaneFollower?.currentEdgeId).not.toBeNull();
  });

  it('exposes staged inbound source-buffer tasks at yellow-grid docks on startup', () => {
    const scenario = createInboundMvpBaselineScenario();
    const sim = new ShuttleSimCore(scenario);
    ...
    expect(inboundTasks).toHaveLength(8);
    expect(queuedInboundTasks).toHaveLength(8);
    expect(inboundTasks.map((task) => task.pickupNodeId)).toEqual([
      'module-01-spine-top-a',
      'module-02-spine-top-a',
      'module-01-spine-top-a',
    ...
    expect(inboundTasks.slice(2).every((task) => task.waitReason === 'inbound-lift-fifo-wait')).toBe(true);
  });
  ...
  it('keeps a later same-lift inbound queue task from blocking the earlier pickup task', () => {
    ...
    if (task8) {
      ...
      if (task8.vehicleId) {
        ...
        expect(task8.waitReason).toMatch(/^(?:inbound-lift-fifo-wait|vehicle-unavailable)$/);
      }
    } else {
  ...
  it('keeps unready top-lift inbound work from parking on the pickup stop and gridlocking traffic', () => {
    ...
    expect(state.kpis.completedInbound).toBeGreaterThanOrEqual(7);
  ...
  it('keeps a staged same-lift inbound successor in queue until its deeper predecessor clears', () => {
    ...
    const stagedTask = state.tasks.find((task) => task.id === stagedVehicle?.taskId);

    expect(stagedTask?.pickupNodeId).toMatch(/^module-\d{2}-spine-top-a$/);
    expect(stagedTask?.liftPortNodeId).toMatch(/^lift-\d{2}-inbound$/);
    expect(stagedVehicle?.plannedGoalNodeId).toMatch(/^(?:column-top-[ab]-c\d+|module-\d{2}-spine-top-a)$/);
    expect(stagedVehicle?.plannedRouteNodeIds.at(-1)).toMatch(/^(?:column-top-[ab]-c\d+|module-\d{2}-spine-top-a)$/);
    expect(stagedVehicle?.plannedRouteNodeIds).not.toContain('lift-01-inbound-queue-01-service-exit');
  });

  it('does not dispatch empty outbound dropoff clearance through the lift workcell when dropoff is a yellow-grid dock', () => {
    const sim = new ShuttleSimCore(createInboundOutboundDemoScenario());
    sim.start();
    ...
    expect(clearanceRoute).toBeNull();
  });
  ...
  it('moves a later same-lift task off the pickup point when the earlier load is still first', () => {
    ...
    expect(vehicle?.plannedGoalNodeId).toBe('column-top-a-c06');
  ...
  it('keeps a later inbound queue blocker at the queue meter so the earlier pickup can pass', () => {
    ...
    expect(laterVehicle?.plannedGoalNodeId).toBe('column-top-a-c06');
    expect(laterVehicle?.routeNodeIds).not.toContain('lift-01-inbound-queue-01-service-exit');
    expect(laterVehicle?.waitReason).not.toBe('inbound-source-not-at-pickup');
  });

  it('stages later inbound work on a yellow-grid approach node instead of the lift workcell', () => {
    ...
    expect(internals.topLiftInboundQueueNodeIdForTask(laterTask, laterVehicle)).toBe('column-top-b-c06');
  });
  ...
  it('keeps top-lift inbound allocation bounded by source-buffer depth while a SKU column is active', () => {
  ...
  it('creates top-lift outbound work against the lift-side service dock', () => {
    const scenario = createInboundOutboundDemoScenario({
      vehicles: { count: 1 },
      taskGeneration: {
        inboundRatePerHour: 0,
    ...
    expect(outboundTask?.dropoffNodeId).toBe('module-01-spine-bottom-b');
    expect(outboundTask?.liftPortNodeId).toBe('lift-01-outbound');
    const outboundDropoffNode = scenario.layout.nodes.find((node) => node.id === outboundTask?.dropoffNodeId);
    expect(outboundDropoffNode?.id).toBe('module-01-spine-bottom-b');
  });
  ...
  it('clears an outbound dropoff through the dedicated queue exit instead of entering buffer access', () => {
    const sim = new ShuttleSimCore(createInboundOutboundDemoScenario({
      vehicles: { count: 1 },
      taskGeneration: {
        inboundRatePerHour: 0,
    ...
    expect(clearanceEvent?.details.route).toBe('lift-01-outbound-queue-01-service-exit>lift-01-outbound-queue-02-service-exit>lift-01-outbound-queue-03-service-exit>lift-01-outbound-queue-03-entry-access>column-bottom-b-c06');
  });
  ...
  it('lets an empty post-dropoff shuttle clear through loaded storage cells when the column exit is open', () => {
    const scenario = createInboundOutboundDemoScenario({
      vehicles: { count: 2 },
      taskGeneration: {
        inboundRatePerHour: 0,
  ...
  it('moves an empty top-lift queue blocker deeper when a loaded shuttle exits the lift service lane', () => {
    ...
    sim.setVehicleTaskForTest('SH-01', null, true);
    sim.setVehicleRouteForTest('SH-02', ['parking-lift-01-inbound-queue']);
  ...
  it('routes loaded outbound from storage to a yellow-grid lift dock without falling back through stored loads', () => {
    const scenario = createInboundOutboundDemoScenario({
      taskGeneration: {
        inboundRatePerHour: 0,
        outboundRatePerHour: 0,
    ...
    const task = {
      id: 'outbound-yellow-dock-clear-side',
      kind: 'outbound',
      state: 'in-progress',
      createdAtSec: 0,
      assignedAtSec: 0,
      ...
      dropoffNodeId: 'module-01-spine-bottom-b',
      liftPortNodeId: 'lift-01-outbound',
      loadId: 'route-load',
      vehicleId: 'SH-01',
      replanCount: 0,
      waitReason: null
    ...
    const route = internals.agentRefreshLoadedOutboundRouteToDropoff('storage-r07-c02', task);

    expect(route?.slice(0, 2)).toEqual(['storage-r07-c02', 'column-middle-c02']);
    expect(route?.at(-1)).toBe('module-01-spine-bottom-b');
    expect(route).not.toContain('storage-r06-c02');
    expect(route).not.toContain('storage-r05-c02');
  ...
  it('keeps later inbound column tasks queued until deeper predecessors finish', () => {
    const scenario = createInboundOutboundDemoScenario({
      vehicles: { count: 2 },
      taskGeneration: {
        inboundRatePerHour: 0,
    ...
    const sim = new ShuttleSimCore(scenario);
    sim.addLoadForTest({ id: 'column-predecessor-load', state: 'carried', nodeId: null, vehicleId: 'SH-01', weightKg: 100 });
    sim.addLoadForTest({ id: 'column-successor-load', state: 'carried', nodeId: null, vehicleId: null, weightKg: 100 });
    sim.addTaskForTest({
    ...
    const internals = sim as unknown as {
      ...
      inboundPickupDispatchGoalNodeId(task: unknown): string;
      topLiftInboundQueueNodeIdForTask(task: unknown): string | null;
    };
    const successor = internals.tasks.find((task) => task.id === 'column-successor-task')!;
    const queueNodeId = internals.topLiftInboundQueueNodeIdForTask(successor);

    expect(internals.inboundTaskShouldWaitBeforePickup(successor)).toBe(true);
    expect(internals.inboundPickupDispatchGoalNodeId(successor)).toBe(queueNodeId);
    expect(internals.inboundPickupDispatchGoalNodeId(successor)).not.toBe('lift-01-inbound-queue-01-service-exit');
  });
  ...
  it('lets taskless no-stop middle clearance pass before off-aisle middle entrants', () => {
    const scenario = createInboundOutboundDemoScenario({
      vehicles: { count: 3 },
      taskGeneration: {
        inboundRatePerHour: 0,
    ...
    const sim = new ShuttleSimCore(scenario);
    sim.addLoadForTest({ id: 'middle-entrant-load', state: 'carried', nodeId: null, vehicleId: 'SH-03', weightKg: 100 });
    sim.addTaskForTest({
      id: 'middle-entrant-outbound',
      kind: 'outbound',
      state: 'in-progress',
      createdAtSec: 0,
      assignedAtSec: 0,
      startedAtSec: 0,
      completedAtSec: null,
      pickupNodeId: 'storage-r07-c24',
      dropoffNodeId: 'module-02-out-lift-buffer-1',
      loadId: 'middle-entrant-load',
      vehicleId: 'SH-03',
      replanCount: 0,
      waitReason: null
    });
    sim.setVehicleRouteForTest('SH-01', ['column-middle-c21', 'column-middle-c22', 'storage-r07-c22']);
    sim.setVehicleWaitingForTest('SH-01', {
    ...
    sim.setVehicleTaskForTest('SH-03', 'middle-entrant-outbound', true);
    sim.setVehicleWaitingForTest('SH-03', {
      targetNodeId: 'column-middle-c24',
      waitReason: 'middle-aisle-opposing-claim',
      blockingVehicleId: 'SH-01',
      waitingSinceSec: 0
    });
    ...
    const clearingVehicle = internals.vehicles.find((vehicle) => vehicle.id === 'SH-01');

    expect(clearingVehicle).toBeDefined();
    expect(internals.agentRefreshMoveBlocker(
      clearingVehicle!,
      'column-middle-c22',
      ['column-middle-c21', 'column-middle-c22', 'storage-r07-c22']
    )).toBeNull();
  });

  it('routes taskless post-dropoff exits around stored loads to the alternate yellow-grid exit', () => {
    const scenario = createInboundOutboundDemoScenario({
      vehicles: { count: 1 },
      taskGeneration: {
        inboundRatePerHour: 0,
    ...
    for (const row of [4, 5, 6, 7]) {
      sim.addLoadForTest({
        id: `post-dropoff-load-r${row}`,
        state: 'stored',
        nodeId: `storage-r0${row}-c21`,
        vehicleId: null,
        weightKg: 100
      });
    }
    sim.setVehicleRouteForTest('SH-01', ['storage-r04-c21']);
    ...
    const routeNodeIds = internals.postDropoffColumnExitRoute({ id: 'SH-01', currentNodeId: 'storage-r04-c21' }) ?? [];
    expect(routeNodeIds).not.toContain('storage-r05-c21');
    expect(routeNodeIds).not.toContain('storage-r06-c21');
    expect(routeNodeIds).not.toContain('storage-r07-c21');
    expect(routeNodeIds).toEqual([
      'storage-r04-c21',
      'storage-r03-c21',
      'storage-r02-c21',
      'storage-r01-c21',
    ...
    expect(internals.layoutNode(routeNodeIds.at(-1) ?? '')).toBeDefined();
    expectRouteHopsAdjacent(scenario, routeNodeIds);
  });
